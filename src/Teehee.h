...
class Teehee {
private:
    std::string api_base = "https://teehee.dev/api";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("teehee.dev"));
...
public:
    ...
    pplx::task<json::value> get_random_joke() {
        return make_api_call("/joke","GET");
    }

    pplx::task<json::value> get_joke_by_id(const std::string& id) {
        return make_api_call("/joke/" + utility::conversions::to_utf8string(web::uri::encode_data_string(utility::conversions::to_string_t(id))),"GET");
    }
};
